...
<form method="POST" action="cek.php?modul=anggota&act=update">
		<input type="hidden" name="id" value="<?php echo $r['id_anggota'];?>">
<table class="table table-bordered">
			<tr>
				<td>Id anggota</td>
				<td><input type="text" name="id"  class="form-control"  class="input" readonly="readyonly" value="<?php echo $r['id_anggota']; ?>"></td>
			</tr>
			<tr>
				<td>nama anggota</td>
				<td><input type="text" name="namaanggota" 
					...
					class="form-control" value="<?php echo $r['namaanggota']; ?>"/></td>
				</tr>
				<tr><td>JENIS KELAMIN</td><td>
					<input type="radio" pattern="[a-zA-Z ]+" autofocus required 
					oninvalid="this.setCustomValidity('Input Hanya boleh Huruf A-Z')" 
					name="gender"  value="L" <?php if($r['jk']=='L') echo "checked='checked'" ?>> L
					<input type="radio" 
					pattern="[a-zA-Z ]+" autofocus required 
					oninvalid="this.setCustomValidity('Input Hanya boleh Huruf A-Z')"
					value="P"  name="gender" <?php if($r['jk']=='P') echo "checked='checked'" ?>> P</td></tr>
					<tr>
						<td>Tempat lahir</td>
						<td><input type="text" name="tempat_lahir" 
							pattern="[a-zA-Z ]+" autofocus required 
							oninvalid="this.setCustomValidity('Input Hanya boleh Huruf A-Z')"
							class="form-control" value="<?php echo $r['tempat_lahir']; ?>"/></td>
						</tr>
						<tr>
							<td>Tgl lahir</td>
							<td><input type="text" name="tgl_lahir"  class="form-control" value="<?php echo $r['tgl_lahir']; ?>"/></td>
							</tr>
							<tr>
								<td>alamat</td>
								<td><input type="text" name="alamat" 
									pattern="[a-zA-Z ]+" autofocus required 
									oninvalid="this.setCustomValidity('Input Hanya boleh Huruf A-Z')"
									class="form-control" value="<?php echo $r['alamat']; ?>"/></td>
								</tr>
								<tr>
									<td>hp</td>
									<td><input type="text" name="hp" pattern="[0-9]+" autofocus required 
										oninvalid="this.setCustomValidity('Input Hanya boleh Angka')"
										maxlength="12"   class="form-control" value="<?php echo $r['hp']; ?>"/></td>
									</tr>
									<tr>
